...
Public Class CWorksheet
	...
	Function GetCellValue(ByVal cl As Cell) As String
		...
		ElseIf cl.DataType IsNot Nothing Then
			If cl.DataType.Value = CellValues.SharedString Then
                ...
                If ssi.InnerText IsNot Nothing Then
	...
	Private Function GetCellvalue(ByVal sheetData As SheetData, ByVal icol As Integer, ByVal irow As Integer) As CellValue
		...
		If cell.DataType IsNot Nothing Then
			If cell.DataType.Value = CellValues.SharedString Then
                ...
                If ssi.Text IsNot Nothing Then
                    Dim cv As New CellValue(ssi.Text.InnerText)
                Else
                    Dim cv As New CellValue(Nothing)
                End If
			End If
		End If
	...
	Private Function GetCurrentRegion() As String
        ...
        Dim erange = Nothing
        ' Here we have a slight issue of malformed SheetDims. This can be generated by some office apps. 
        'I have no idea why. But instead of A1:C65 we get 1:65. How do we detect these. 
        'For now lets use the lack of letters as an indicator
        If sheetDims IsNot Nothing AndAlso (CRange.ColumnNameRegex.Matches(sheetDims.Reference.InnerText).Count > 0) Then
