Public Module ResultsProcessingUtilities
    ...
    Private Function SecondSemesterProcessRegularStudent(studEnr As BatchEnrollment) As GPAwRecomm
        ...
        Dim PreviousCGPA As Double? = GetPreviousCGPA(studEnr)
        ...
        ElseIf CountABs > 0 Then
            ...
            If Double.IsNaN(GPA) And (CountFs + CountDs) = 0 Then
                gpw.Comment = String.Format("Sub ({0})", CountABs)
            ElseIf Double.IsNaN(GPA) And (CountFs + CountDs) < Math.Ceiling(CountSubjects / 3) + 1 Then
    ...
    Public Function AssignGrade(mk As MarksExamCW) As GradeTotal
        ...
        AssignGrade.Mark = mk
        Dim CWFraction As Double = mk.OfferedCourse.CourseWorkFraction
        Dim ExFraction As Double = mk.OfferedCourse.ExamFraction
    ...
    Private Function GetPreviousCGPA(studEnr As BatchEnrollment) As Double?
    ...
    Private Function EvaluateCGPA(GPA As Double, PreviousCGPA As Double?, CurrentClass As Integer) As Double?
    ...
    Private Function GNiceRecomm(CGPA As Double?, PreviousCGPA As Double?, YRecomm As RecommTypeEnum, ByRef Comment As String) As RecommTypeEnum?
