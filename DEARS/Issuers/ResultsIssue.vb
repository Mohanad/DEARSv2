...
Public Class ResultsIssue
    ...
    Function GenerateGradingFormula(ByVal Col As Integer, ByVal Row As Integer, ByVal CWHasValue As Boolean) As String
        ...
        Dim tempFor As String = "IF(AND({2}=""--"",{0}>={4}*0.4),""AB"",IF(AND({2}=""--"",{0}<{4}*0.4),""AB*"",IF({0}<{4}*0.4,IF(OR({1}<{5}*0.3,{0}<{4}*0.3),""F*"",""D*""),IF({1}<{5}*0.3,""F"",IF({1}<{5}*0.4,""D"",IF({2}<70,IF({2}<50,""C"",IF({2}<60,""B"",""B+"")),IF({2}<80,""A-"",IF({2}<90,""A"",""A+""))))))))"
        'If CWHasValue Then
